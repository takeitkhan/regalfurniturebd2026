        ...
        if (!empty($cart['items']) && is_array($cart['items'])) {
            ...
            foreach ($cart['items'] as $rKey => $re_price) {
                ...
                if ($reProduct && $reProduct->is_active == 1) {
                    $re_price['purchaseprice'] = $reProduct->product_price_now;
                    $re_price['item']['purchaseprice'] = $reProduct->product_price_now;
                    $currentTotal += (((float)$reProduct->product_price_now) * $re_price['qty']);
    ...
    public function orderDetails(Request $request)
    {
        ...
        foreach ($db_order_details_by_random as $order) {
            ...
            $order_details[] = [
                ...
                'image_url' => $order->firstImage->icon_size_directory ?? null,
